"""
LTF files consist of tokenized documents. 
A simple script for reading ltf files is available at: 
https://github.com/wilburOne/ACE_ERE_Scripts/blob/master/ltf2sent.py

./training/data/ltf/ -- "logical text format" (ltf) derived from the rsd
./training/annotations/edl.tab -- annotation output. Ground trouth. 
"""
...
def ltf_to_plaintext(ltf_str):
    ''' WORKING. Has the same char offsets as the laod_ltf function. '''
    doc_tokens = []
    root = ET.fromstring(ltf_str)
    doc_id = root.find('DOC').get('id')
    original_text_list = []
    for seg in root.find('DOC').find('TEXT').findall('SEG'):
        sent_tokens = []
        seg_text = seg.find('ORIGINAL_TEXT').text
        original_text_list.append(seg_text)
        seg_start = int(seg.get('start_char'))
        seg_end = int(seg.get('end_char'))
        for token in seg.findall('TOKEN'):
            token_text = token.text
